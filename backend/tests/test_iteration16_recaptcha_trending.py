...
class TestSubscriptionCheckout:
    ...
    def test_checkout_paid_plan_creates_stripe_url(self, auth_token):
        ...
        response = requests.post(f"{BASE_URL}/api/subscriptions/checkout", 
            json={"plan": "rise", "origin_url": "https://artist-hub-220.preview.emergentagent.com"},
    ...
    def test_checkout_free_plan_no_stripe(self, auth_token):
        ...
        response = requests.post(f"{BASE_URL}/api/subscriptions/checkout", 
            json={"plan": "free", "origin_url": "https://artist-hub-220.preview.emergentagent.com"},
            headers={"Authorization": f"Bearer {auth_token}"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_checkout_invalid_plan(self, auth_token):
        ...
        response = requests.post(f"{BASE_URL}/api/subscriptions/checkout", 
            json={"plan": "invalid_plan", "origin_url": "https://artist-hub-220.preview.emergentagent.com"},
            headers={"Authorization": f"Bearer {auth_token}"})
