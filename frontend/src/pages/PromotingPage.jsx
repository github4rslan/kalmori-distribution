    ...
    <PublicLayout>
      <div className="max-w-2xl mx-auto" data-testid="promoting-page">
        ...
        <div className="px-4 grid grid-cols-2 gap-3 mb-8">
          {channels.map((ch, i) => (
            <div key={i} className="bg-[#111] rounded-xl p-4 text-center border border-[#222]" data-reveal data-reveal-variant={['zoom','flip','tilt','rise'][i % 4]} data-reveal-delay={String((i % 4) + 1)}>
        ...
        <div className="px-4 space-y-4 pb-8">
          ...
          {packages.map((pkg, i) => (
            <div key={i} className={`bg-[#111] rounded-2xl p-6 text-center border-2 ${pkg.highlight ? 'border-[#E040FB]' : 'border-[#222]'}`} data-testid={`promo-${pkg.name.toLowerCase()}`} data-reveal data-reveal-variant={i === 0 ? 'left' : i === 1 ? 'zoom' : 'right'} data-reveal-delay={String((i % 3) + 1)}>
